...
static int minttemp_raw[PROBES] = ARRAY_BY_PROBES( HEATER_0_RAW_LO_TEMP , HEATER_1_RAW_LO_TEMP , HEATER_2_RAW_LO_TEMP );
static int maxttemp_raw[PROBES] = ARRAY_BY_PROBES( HEATER_0_RAW_HI_TEMP , HEATER_1_RAW_HI_TEMP , HEATER_2_RAW_HI_TEMP );
static int minttemp[PROBES] = ARRAY_BY_PROBES( 0, 0, 0 );
  ...
  for(int e = 0; e < PROBES; e++) 
  {
    ...
    if(watchmillis[e] && millis() - watchmillis[e] > WATCH_TEMP_PERIOD)
    {
      ...
      if(fabs(current_temperature[0] - redundant_temperature) > MAX_REDUNDANT_TEMP_SENSOR_DIFF) {
        ...
        if(IsStopped() == false) {
          SERIAL_ERROR_START;
          SERIAL_ERRORLNPGM("PROBE switched off. Temperature difference between temp sensors is too high !");
...
static float analog2temp(int raw, uint8_t e) {
...
#endif
  {
      SERIAL_ERROR_START;
      SERIAL_ERROR((int)e);
      SERIAL_ERRORLNPGM(" - Invalid PROBE number !");
      kill();
      return 0.0;
  } 
...
void max_temp_error(uint8_t e) {
  ...
  if(IsStopped() == false) {
    ...
    SERIAL_ERRORLNPGM(": PROBE switched off. MAXTEMP triggered !");
...
void min_temp_error(uint8_t e) {
  ...
  if(IsStopped() == false) {
    SERIAL_ERROR_START;
    SERIAL_ERRORLN((int)e);
    SERIAL_ERRORLNPGM(": PROBE switched off. MINTEMP triggered !");
...
#endif 
#if (PROBES > 1) || defined(HEATERS_PARALLEL)
