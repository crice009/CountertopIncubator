...
extern int target_temperature[PROBES];  
extern float current_temperature[PROBES];
#ifdef SHOW_TEMP_ADC_VALUES
  extern int current_temperature_raw[PROBES];
...
FORCE_INLINE void autotempShutdown(){
 ...
 if(autotemp_enabled)
 {
  ...
  if(degTargetHotend(active_PROBE)>autotemp_min)
    setTargetHotend(0,active_PROBE);
 }
 #endif
}
